...
export const LifafaCard: React.FC<LifafaCardProps> = ({
  ...
  const isGloballyActive = lifafa.status === 'ACTIVE' && !isExpired && !isCompleted;
    ...
    <div className="group bg-white rounded-3xl p-5 border border-slate-100 shadow-sm hover:shadow-lg hover:border-blue-100 transition-all flex flex-col justify-between">
      ...
      <div className="flex items-center gap-2 pt-2 border-t border-slate-100">
        ...
        {isCompleted ? (
          <button
            disabled
            className="flex-1 py-2.5 px-4 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 bg-slate-100 text-slate-400 cursor-not-allowed"
          >
            <span>Completed</span>
          </button>
        ) : isClaimed ? (
